...
type neuron =
{
  ...
  w : float array;
(* un tableau de float qui contient des valeurs d'activations *)
}
...
let save network file =
  ...
  Marshal.to_channel f network [];
 (* Marshal permet d'avoir une sérialization automatique *)
  close_out f
...
(* ne fonctionne que sur des images en niveau de gris et
 renvoie un flotant entre 0 et 1 *)
let get_color image x y = 
  let (c,_,_) = image.(x).(y) in
  (float c) /. 255.

(* get_dims renvoie les dimensions d'une image sous la forme d'une matrice *)
let get_dims_mat image =
  (Array.length image, Array.length image.(0))

(* Watch donne au réseau de neurones l'image en input *)
...
(* Learn apprend au réseau de neurones un caractère *)
